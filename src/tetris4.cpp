// 블록이 가장 왼쪽으로 붙어있을 때 한 번 더 왼쪽 방향키를 누르면 오른쪽 벽으로 나온다.
...
int ShiftLeft()	// 왼쪽 이동
{
	...
	{	
		...
		for (int i = 0; i < 4; i++)
		{
			if (blockModel[block_id][i][3] == 1)
				flag2 = 1;
		}

		if (flag1 == 0)
		{
			curPosX += GBOARD_WIDTH*2 - 2;
			SetCurrentCursorPos(curPosX, curPosY);
			ShowBlock(blockModel[block_id]);
		}
