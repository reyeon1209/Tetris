// 순번의 끝자리가 4, 8인 경우 항상 슈퍼블록이 떨어진다.
// ■■■
// ■■■
// ■■■
// 슈퍼블록은 다른 블록을 관통해서 지나가며 지나간 자리의 블록은 그대로 존재한다.
// 슈퍼블록은 항상 최하단에 위치한다.
...
int DetectCollision(int posX, int posY, char blockInfo[4][4])	// 충돌 검사
{
	...
	if (block_id == 28)
	{
		for (int x = 0; x < 4; x++)
		{
			for (int y = 0; y < 4; y++)
			{
				if ((blockInfo[y][x] == 1) && (gameBoardInfo[arrY+y][arrX+x] == 2))	// 2차원배열 인덱스
					return 0;	// 충돌하면 0
			}
		}
	}
	...
	else
	{
		for (int x = 0; x < 4; x++)
		{
			...
			for (int y = 0; y < 4; y++)
				if (blockInfo[y][x] == 1)
				{
					if (gameBoardInfo[arrY+y][arrX+x] == 1)	// 2차원배열 인덱스
						return 0;	// 충돌하면 0

					if (gameBoardInfo[arrY+y][arrX+x] == 2)
						return 0;
				}
			}
		}
